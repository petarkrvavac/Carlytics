...
export function DesktopActiveAssignmentsTable({ assignments }: DesktopActiveAssignmentsTableProps) {
  ...
  useEffect(() => {
    ...
    const refreshKey = `desktop-release:${releaseState.message}`;
    ...
    router.refresh();
  }, [releaseState.message, releaseState.status, router]);
              ...
              <tr key={assignment.id} className="border-b border-border/60 last:border-0">
                ...
                <td className="px-2 py-3 text-right align-top">
                  {expandedAssignmentId === assignment.id ? (
                    <form action={releaseAction} className="ml-auto w-56 space-y-2 rounded-lg border border-border bg-surface p-2">
                    ...
                    <button
                      ...
                      className="inline-flex h-8 items-center justify-center rounded-lg border border-rose-400/45 bg-rose-100 px-3 text-xs font-semibold text-rose-800 transition hover:border-rose-500/70 hover:bg-rose-200 dark:bg-rose-500/15 dark:text-rose-200 dark:hover:bg-rose-500/25"
                    >
                      Razduži vozilo
                    </button>
                  )}
